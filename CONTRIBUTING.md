...
```bash
git clone https://github.com/CleatSquad/php-error-boundary.git
cd php-error-boundary
composer install
```
...
- Never let a mapper leak a raw exception message, file path, or class name to
  the client by default — that's the boundary's entire purpose.
